...
float* retrieve_data(wav_info *w, char* file_path) {

	int16_t *idata;

	idata = open_file_read_data(w, file_path);
...
int16_t* open_file_read_data(wav_info *w, char* file_path) {

/*
	User now has to pass the file path from main.
...
*/
	...
	while(1) 
	{
		...
		else if(strcmp(file_path, "EXIT") == 0)
		{
			printf("\n");
